...
class ProductsController extends Controller
{
    ...
    public function product_order_view($id)
    {
        ...
        return view(view: 'backend.products.product-order-view')->with(['product_order' => $product]);
    }
    ...
    {
        $dataUrl = "https://www.safeer.com/products/{$id}";
        $qrCodeUrl = "https://api.qrserver.com/v1/create-qr-code/?size=1080x1080&data={$dataUrl}";

        // Using GuzzleHttp to send a GET request to the QR API
        $client = new \GuzzleHttp\Client();
        $response = $client->get($qrCodeUrl);

        // Assuming the response body is the image content
        $imageContent = $response->getBody()->getContents();

        // Generate a response that forces download
    ...
    {
        $ids = $request->input('bulk_delete'); // Array of product IDs
        ...
        if ($zip->open($zipPath, ZipArchive::CREATE) === TRUE) {
            foreach ($ids as $id) {
                $dataUrl = "https://www.yourwebsite.com/products/{$id}";
                $qrCodeUrl = "https://api.qrserver.com/v1/create-qr-code/?size=1080x1080&data={$dataUrl}";
                $imageContents = @file_get_contents($qrCodeUrl);
                if ($imageContents === false) {
                    continue; // Skip if the QR code wasn't fetched correctly
                }
                $zip->addFromString("product_{$id}.png", $imageContents);
            }
            $zip->close();
        } else {
        ...
        if (file_exists($zipPath)) {
            return response()->json(['file' => asset($zipFileName)]);
        } else {
