...
class FrontendController extends Controller
{
    public function index()
    {
        ...
        $all_travels = Travel::with('lang_front')->take(4)->get();
    ...
    public function blog_page()
    {
        ...
        $all_blogs = Blog::with('lang_front')->where('status', 'publish')->paginate(9);
    ...
    {
        $all_travels = Travel::with('lang_front')->orderBy('id', 'DESC')->paginate(10);
        return view('frontend.pages.travel')->with([
            'all_travels' => $all_travels
        ]);
    }

    public function travel_single_page($id)
    {
        $travel = Travel::with('lang_front')->find($id);
        $average_ratings = TravelRatings::Where('travel_id', $id)->pluck('ratings')->avg();

        return view('frontend.pages.travel-single')->with([
            'travel' => $travel,
    ...
    public function products(Request $request)
    {
        ...
        $query = Products::with([
            'lang_front' => function ($query) use ($search_term) {
    ...
    public function travel_ratings(Request $request)
    {
        $this->validate($request, [
            'travel_id' => 'required',
            'ratings' => 'required',
        ...
        $existing_rating = TravelRatings::where(['travel_id' => $request->travel_id, 'user_id' => auth()->guard('web')->user()->id])->first();
        if (!empty($existing_rating)) {
            return redirect()->back()->with(['msg' => __('You have already rated this travel'), 'type' => 'danger']);
        }
        TravelRatings::create([
